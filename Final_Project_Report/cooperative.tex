...
\begin{itemize}
	\item Case 1: Horizontal attitude disabled and same initial and target positions as stated in the exercise description;
	\item Case 2: Horizontal attitude enabled and same initial position but different target position, which is the following 
	\begin{gather*}
		\begin{bmatrix} 10.5 & 37.5 & -38 & \pi/3 & \pi/3 & 0\end{bmatrix}^\top
	\end{gather*}
	\item Case 3: Horizontal attitude enabled and different initial and target position, which are the following 
	\begin{gather*}
...
{\large \textbf{Case 1}} \\
In this case the behaviour is normal, the robot reaches the target position and attitude without problems.
\begin{figure}[H]
	\centering
...
\begin{figure}[H]
	...
	\caption{Here we can see that the robot reaches the goal position and that the misalignment vector shrinks to zero.}
	\label{fig:case1c}
\end{figure}

{\large \textbf{Case 2}} \\
In this case the target position has roll and pitch angles both equal to $ \pi/3 $, so the Vehicle Position and Attitude task tries to rotate the robot in order to fullfill its objective, but, since the Horizontal Attitude task is enabled with higher priority, the robot can't reach the final position. In this case we have two conflicting tasks, but since the Horizontal Attitude one has higher priority the robot will try to stay as horizontal as possible: nevertheless, the robot will be a bit tilted since this task is of inequality type.
\begin{figure}[H]
	\centering
...
\begin{figure}[H]
	\centering
	\subfigure[Heading of the vehicle when it approches the ground with gain = \(0.05\)]
...
\begin{figure}[H]
	\centering
	\subfigure[Heading of the vehicle when it approches the ground with gain = \(0.2\)]
...
\begin{figure}[H]
	\centering
	\subfigure[Heading of the vehicle when it approches the ground with gain = \(0.4\)]
...
\begin{figure}[H]
	\centering
	\subfigure[Heading of the vehicle when it approches the ground with gain = \(0.7\)]
...
%What we thought is to add a new Action in between the two actual ones that is activated when the vehicle reaches the goal position 
