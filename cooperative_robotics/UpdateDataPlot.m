...
%% Plot vehicle position, initial position and goal position frames and misalignment vector
i = [1 0 0]';
j = [0 1 0]';
k = [0 0 1]';
...
%   Exercise 6
glob_tool_pos = uvms.wTv*uvms.vTt;
plt.t_x(loop) = glob_tool_pos(1,4);
...
% Exercise 6 desired joint values
plt.desired_arm_pos(:,loop) = uvms.armPrefPos;

% Exercise 6 manipulability
plt.mu(loop) = sqrt( det(uvms.bJe*uvms.bJe') );

% Exercise 6 dexterity
plt.dex(loop) = norm(uvms.bJe,2)*norm( pinv(uvms.bJe),2);
